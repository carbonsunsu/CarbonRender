...
private:
	...
	float3 PerlinFade(float3 a);
	float PerlinGrab(int a, float3 b);
	float PerlinPermute(int a);
	float PerlinNoise(float3 uv);
	float Fbm(float3 uv);

	float3 WorleyPermute(float3 a);
	float4 WorleyPermute(float4 a);
	float3 Length(float3 a, float3 b, float3 c);
	float4 Length(float4 a, float4 b, float4 c);
	float WorleyNoise(float3 uv);
	float WorleyNoiseFast(float3 uv);

	float3 CurlNoise(float3 uv);
public:
	void Init();
};
